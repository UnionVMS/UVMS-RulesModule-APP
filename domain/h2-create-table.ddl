...
create table rules.rawmessage (id int8 not null, raw_message text not null, primary key (id))
...
create table rules.validationmessage (id int8 not null, br_id varchar(255) not null, error_type varchar(255) not null, level varchar(255) not null, message text not null, raw_message_id int8, primary key (id))
alter table rules.externalRule add constraint UK_mvma8gfn7i31i5c936g89kpv6 unique (br_id)
...
alter table rules.externalRule add constraint FK_ekj1ou268vd1cbojpyg687j2c foreign key (template_id) references rules.template
